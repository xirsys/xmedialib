...
defmodule XMediaLib.GenRtpChannel do
  ...
  def open(port), do:
    open(port, [])
  def open(port, params), do:
    GenServer.start_link(__MODULE__, [params ++ port: port], [])
  ...
  def handle_call(:get_stats, _, %__MODULE__{rtp: port, ip: ip, rtp_port: rtp_port, rtcp_port: rtcp_port, local: local, sr: sr, rr: rr} = state) do
    <<ssrc::32, type::8, rx_bytes::32, rx_packets::32, tx_bytes::32, tx_packets::32, tx_bytes2::32, tx_packets2::32>> = port_control(port, 5, <<>>)
    {:reply, {local, {ip, rtp_port, rtcp_port}, ssrc, type, rx_bytes, rx_packets, tx_bytes, tx_packets, tx_bytes2, tx_packets2, sr, rr}, state}
  end

  def handle_call({:rtp_subscriber, {:set, subscriber}}, _, %__MODULE__{peer: nil} = state), do:
    {:reply, :ok, %__MODULE__{state | rtp_subscriber: subscriber}}
  def handle_call({:rtp_subscriber, {:set, nil}}, _, state), do:
    {:reply, :ok, %__MODULE__{state | rtp_subscriber: nil}}
  def handle_call({:rtp_subscriber, {:set, subscriber}}, _, %__MODULE__{peer: {posix_fd, {i0, i1, i2, i3} = ip, port}} = state) do
    GenServer.cast(subscriber, {:set_fd, <<posix_fd::32, port::16, 4::8, i0::8, i1::8, i2::8, i3::8>>})
    {:reply, :ok, %__MODULE__{state | rtp_subscriber: subscriber}}
  end
  def handle_call({:rtp_subscriber, {:set, subscriber}}, _, %__MODULE__{peer: {posix_fd, {i0, i1, i2, i3, i4, i5, i6, i7} = ip, port}} = state) do
    GenServer.cast(subscriber, {:set_fd, <<posix_fd::32, port::16, 6::8, i0::16, i1::16, i2::16, i3::16, i4::16, i5::16, i6::16, i7::16>>})
    {:reply, :ok, %__MODULE__{state | rtp_subscriber: subscriber}}
  end
  def handle_call({:rtp_subscriber, {:add, subscriber}}, _, %__MODULE__{rtp_subscriber: old_subscriber} = state), do:
    {:reply, :ok, %__MODULE__{state | rtp_subscriber: append_subscriber(old_subscriber, subscriber)}}

  def handle_call(:get_phy, _, %__MODULE__{rtp: fd, ip: ip, rtp_port: rtp_port, rtcp_port: rtcp_port, local: local} = state), do:
    {:reply, {fd, local, {ip, rtp_port, rtcp_port}}, state}
  ...
  def handle_cast({:keepalive, :enable}, state), do:
    {:noreply, %__MODULE__{state | keepalive: true}}
  def handle_cast({:keepalive, :disable}, state), do:
    {:noreply, %__MODULE__{keepalive: false}}
  ...
  def terminate(reason, %__MODULE__{rtp: port, encoder: encoder, decoder: decoder}) do
    ...
    IO.puts "gen_rtp #{inspect self()}: terminated due to reason [#{inspect reason}] (allocated #{bytes} bytes)"
  end
  ...
  def handle_info({:init, params}, state) do
    ...
    case ip_addr do
      {i0, i1, i2, i3} ->
        :erlang.port_control(port, 1, <<ip_port::16, 4::8, i0::8, i1::8, i2::8, i3::8, timeout_early::32, timeout_main::32>>)
        <<i0::8, i1::8, i2::8, i3::8, rtp_port::16, rtcp_port::16>> = :erlang.port_control(port, 2, <<>>)
      {i0, i1, i2, i3, i4, i5, i6, i7} ->
        :erlang.port_control(port, 1, <<ip_port::16, 6::8, i0::16, i1::16, i2::16, i3::16, i4::16, i5::16, i6::16, i7::16, timeout_early::32, timeout_main::32>>)
        <<i0::16, i1::16, i2::16, i3::16, i4::16, i5::16, i6::16, i7::16, rtp_port::16, rtcp_port::16>> = :erlang.port_control(port, 2, <<>>)
    end
  ...
  # end
    ...
    {fn_decode, fn_encode} = case (ctx != :none) or (transcoding != :none) do
      false ->
        {[], []};
      true ->
    ...
    # FIXME
    {encoder, fn_transcode} = case transcoding do
      :none -> {false, []}
      encoder_desc ->
  ...
  def handle_info({pkt, ip, port}, %__MODULE__{rtp: fd, ip: def_ip, rtp_port: def_port, tx_bytes: tx_bytes, tx_packets: tx_packets} = state) when is_binary(pkt) do
    # If it's binary then treat it like RTP
    send(fd, {self(), {:command, pkt}})
    {:noreply, %__MODULE__{state | tx_bytes: tx_bytes: byte_size(pkt) - 12, tx_packets = tx_packets + 1}}
  end
  def handle_info(
    {%Rtp{ssrc: other_ssrc} = pkt, ip, port}
    %__MODULE__{state | rtp: fd, ip: def_ip, rtp_port: def_port, process_chain_down: chain, other_ssrc: other_ssrc, tx_bytes: tx_bytes, tx_packets: tx_packets} = state
  ) do
    {new_pkt, new_state} = process_chain(chain, pkt, state)
    send(fd, {self(), {:command, new_pkt}})
    {:noreply, %__MODULE__{new_state | tx_bytes: tx_bytes + byte_size(new_pkt) - 12, tx_packets = tx_packets + 1}}
  def handle_info({%Rtp{ssrc: other_ssrc} = pkt, ip, port}, %__MODULE__{state | other_ssrc: nil, zrtp: zrtp_fsm} = state) do
  ...
  def handle_info({%Rtp{ssrc: other_ssrc} = pkt, ip, port}, %__MODULE__{other_ssrc: other_ssrc2} = state) do
    # Changed ssrc on the other side
    IO.puts "gen_rtp ssrc changed from [#{inspect other_ssrc2}] to [#{inspect other_ssrc}] (call transfer/music-on-hold?)"
  ...
  def handle_info({:peer, posix_fd, ip, port}, %__MODULE__{rtp_subscriber: nil} = state), do:
    {:noreply, %__MODULE__{state | peer: {posix_fd, ip, port}}}
  def handle_info({:peer, posix_fd, {i0, i1, i2, i3} = ip, port}, %__MODULE__{rtp_subscriber: subscriber} = state) do
    GenServer.cast(subscriber, {:set_fd, <<posix_fd::32, port::16, 4::8, i0::8, i1::8, i2::8, i3::8>>})
    {:noreply, %__MODULE__{state | peer: {posix_fd, ip, port}}}
  end
  def handle_info({:peer, posix_fd, {i0, i1, i2, i3, i4, i5, i6, i7} = ip, port}, %__MODULE__{rtp_subscriber: subscriber} = state) do
    GenServer.cast(subscriber, {:set_fd, <<posix_fd::32, port::16, 6::8, i0::16, i1::16, i2::16, i3::16, i4::16, i5::16, i6::16, i7::16>>})
    {:noreply, %__MODULE__{state | peer: {posix_fd, ip, port}}}
  end
  ...
  def handle_info({:timeout, _port}, state), do:
    {:stop, :timeout, state}
  ...
  # Handle incoming RTP message
  def process_data(fd, ip, port, <<Rtp.rtp_version()::2, _::7, ptype::7, _::48, ssrc::32, _::binary>> = msg, %__MODULE__{rtp_subscriber: subscriber, sendrecv: sendrecv, process_chain_up: [], rx_bytes: rx_bytes, rx_packets: rx_packets} = state) when ptype =< 34 or 96 =< ptype do
    case sendrecv(ip, port, ssrc, state.ip, state.rtp_port, state.ssrc) do
      true ->
        send_subscriber(subscriber, msg, ip, port)
        RtpUtils.get_codec_from_payload(ptype) == dtmf && do
          {:ok, rtp} = Rtp.decode(msg)
          IO.puts "DTMF: #{inspect RtpUtils.pp(rtp)}"
        end
        %__MODULE__{state | ip: ip, rtp_port: port, ssrc: ssrc, type: ptype, rx_bytes: rx_bytes + byte_size(msg) - 12, rx_packets = rx_packets + 1}
      false ->
        state
    end
  end
  def process_data(fd, ip, port, <<Rtp.rtp_version()::2, _::7, ptype::7, _::48, ssrc::32, _::binary>> = msg, %__MODULE__{rtp_subscriber: subscriber, sendrecv: sendrecv, process_chain_up: chain, rx_bytes: rx_bytes, rx_packets: rx_packets} = state) when ptype =< 34 or 96 =< ptype do
    case sendrecv(ip, port, ssrc, state.ip, state.rtp_port, state.ssrc) do
      true ->
        {new_msg, new_state} = process_chain(chain, msg, state)
        send_subscriber(subscriber, new_msg, ip, port)
        %__MODULE__{new_state | ip: ip, rtp_port: port, ssrc: ssrc, type: ptype, rx_bytes: rx_bytes + byte_size(msg) - 12, rx_packets: rx_packets + 1}
      false ->
        state
    end
  end
  # Handle incoming RTCP message
  def process_data(fd, ip, port, <<Rtp.rtp_version()::2, _::7, ptype::7, _::48, ssrc::32, _::binary>> = msg, %__MODULE__{rtp_subscriber: subscriber, sendrecv: sendrecv, rr: rr0, sr: sr0} = state) when 64 =< ptype and ptype =< 82 do
  ...
  def process_data(fd, ip, port, <<Zrtp.zrtp_marker()::16, _::16, Zrtp.zrtp_magic_cookie()::32, ssrc::32, _::binary>> = msg, %__MODULE__{rtp_subscriber: subscriber, sendrecv: sendrecv} = state) do
    ...
    case sendrecv(ip, port, ssrc, state.ip, state.rtp_port, state.ssrc) do
      true ->
        ...
        case state.zrtp do
          ...
          nil -> send_subscriber(subscriber, zrtp, ip, port)
          zrtp_fsm -> GenServer.cast(self(), {GenServer.call(zrtp_fsm, zrtp), ip, port})
        end
  ...
  def process_data(fd, ip, port, <<Stun.stun_marker()::2, _::30, Stun.magic_cookie()::32, _::binary>> = msg, state), do:
    # FIXME this is a STUN message - we should reply at this level
    state
  # Handle incoming UKNOWN message
  def process_data(_, _, _, _, state), do:
    state
  ...
  def process_chain([], pkt, state), do:
    {pkt, state}
  ...
  def transcode(%Rtp{payload_type: payload_type} = rtp, state = %__MODULE__{encoder: {payload_type, _}}), do:
    {rtp, state}
  def transcode(%Rtp{payload_type: old_payload_type, payload: payload} = rtp, state = %__MODULE__{encoder: {payload_type, encoder}, decoder: {old_payload_type, decoder}}) do
  ...
  def transcode(%Rtp{payload_type: old_payload_type, payload: payload} = rtp, state = %__MODULE__{encoder: {payload_type, encoder}, decoder: {different_payload_type, decoder}}) do
    case Codec.is_supported(RtpUtils.get_codec_from_payload(old_payload_type)) do
      true ->
        IO.puts "New payload #{inspect old_payload_type} found while transcoding (was #{inspect different_payload_type})"
        Codec.close(decoder)
  ...
  def transcode(pkt, state), do:
    {pkt, state}

  def send_subscriber(nil, _, _, _), do:
    :ok
  def send_subscriber(subscribers, data, ip, port) when is_list(subscribers), do:
    subscribers
  ...
  def send_subscriber({type, fd, ip, port}, pkt, _, _), do:
    send(fd, {self(), {:command, pkt}})
  def send_subscriber(subscriber, pkt, _, _), do:
    send(subscriber, {pkt, nil, nil})

  def append_subscriber(nil, subscriber), do: subscriber
  def append_subscriber(subscribers, subscriber) when is_list(subscribers), do:
    subs = for s <- subscribers, s != subscriber, do: s
    subs ++ [subscriber]
  end
  ...
  def get_priv(), do: './priv' # Probably eunit session
end